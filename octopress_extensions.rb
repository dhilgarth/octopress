  ...
  cd "#{source_dir}/#{posts_submodule_root}" do
    ...
    unless status.empty?
      ...
      unless current_branch == "blog"
        puts status
        abort("rake aborted") unless ask("You have uncommitted changes and you are on branch '#{current_branch}' rather than 'blog'. Switching branches with uncommitted changes can lead to conflicts. Are you sure you want to switch?", ['y', 'n']) == 'y'
        system "git checkout blog"
      end
      abort("rake aborted") if ask("#{status}\nThese are your uncommitted changes. Are you sure you want to commit them all?", ['y', 'n']) == 'n'
      system "git add ."
